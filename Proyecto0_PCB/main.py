...
class Main:
    ...
    def on_btn_generar_prog_clicked(self, button):
        ...
        for index, widget in enumerate(self.lista_caracteristicas):
            ...
            if widget.get_visible():
                ...
                if "lab" in Gtk.Buildable.get_name(widget):
                    caract_genotipos[widget.get_text()] = self.builder.get_object("txt_car_" + str(int(Gtk.Buildable.get_name(widget)[-1]))).get_text()
        ...
        set_caract_genotipos = {key.upper(): value for key, value in caract_genotipos.items()}
        ...
        for index, value in enumerate(genotipos_combinacion):
            caracteristica = ""
            for key, val in caract_genotipos.items():
